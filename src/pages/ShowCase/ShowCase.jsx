...
import { useRestaurants, useCollections } from '../../hooks/useRestaurants';
...
let ShowCase = () => {
    ...
    // Prepare filters for API - use the actual page value as category
    const [apiFilters, setApiFilters] = useState({
        category: page, // Use page value directly (order-online, dinning-out, night-life)
        page: 1,
        limit: 20
    });
    ...
    const { restaurants, loading, error, pagination } = useRestaurants(apiFilters);
        ...
        <div className={css.innerDiv6}>
            <div className={css.w7}>
                <div className={css.innerDiv6Title}>
                    {page === orderOnlinePage ? "Delivery Restaurants in New Town" : page === diningOutPage ? "Dine-Out Restaurants in New Town" : "Nightlife Restaurants in New Town"}
                </div>
                <div className={css.innerDiv6Body}>
                    {loading ? (
                        <div style={{padding: '20px', textAlign: 'center'}}>Loading restaurants...</div>
                    ) : error ? (
                        <div style={{padding: '20px', textAlign: 'center', color: 'red'}}>{error}</div>
                    ) : !restaurants || restaurants.length === 0 ? (
                        <div style={{padding: '20px', textAlign: 'center'}}>No restaurants found</div>
                    ) : (
                        restaurants.map((restaurant, id) => {
                            // Use restaurant images if available, fallback to static images
                            const imgSrc = restaurant.images?.[0] || biryaniSCImg;
                            
                            return <ShowcaseCard 
                                key={restaurant._id || id} 
                                promoted={restaurant.featured || false}
                                time={restaurant.deliveryTime || "30"}
                                offB={restaurant.discount > 0}
                                proExtraB={restaurant.featured}
                                off={restaurant.discount?.toString() || "0"}
                                proExtra="40"
